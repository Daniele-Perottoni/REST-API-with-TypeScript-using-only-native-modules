// access the data store (fixtures/store.json)
import fs from "fs";
import path from "path";

// handle requests and reponses
import { ServerResponse, IncomingMessage } from "http";

// access the task structure
import { Task } from "./ITask";
const jsonPath = path.join(__dirname, "../fixtures/store.json");

const getTasks = (req: IncomingMessage, res: ServerResponse) => {
  return fs.readFile(jsonPath, "utf8", (err, data) => {
    // Read the fixtures/store.json file
    // Check out any errors
    if (err) {
      // error, send an error message
      res.writeHead(500, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({
          success: false,
          error: err,
        }),
      );
    } else {
      // no error, send the data
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({
          success: true,
          message: JSON.parse(data),
        }),
      );
    }
  });
};

const addTask = (req: IncomingMessage, res: ServerResponse) => {
  // Read the data from the request
  let data = "";

  req.on("data", (chunk) => {
    data += chunk.toString();
  });

  // When the request is done
  req.on("end", () => {
    let task = JSON.parse(data);

    // Read the fixtures/store.json file
    fs.readFile(jsonPath, "utf8", (err, data) => {
      // Check out any errors
      if (err) {
        // error, send an error message
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({
            success: false,
            error: err,
          }),
        );
      } else {
        // no error, get the current tasks
        let tasks: Task[] = JSON.parse(data);
        // get the id of the latest task
        let latest_id = tasks.reduce(
          (max = 0, task: Task) => (task.id > max ? task.id : max),
          0,
        );
        // increment the id by 1
        task.id = latest_id + 1;
        // add the new task to the tasks array
        tasks.push(task);
        // write the new tasks array to the fixtures/store.json file
        fs.writeFile(jsonPath, JSON.stringify(tasks,null,2), (err) => {
          // Check out any errors
          if (err) {
            // error, send an error message
            res.writeHead(500, { "Content-Type": "application/json" });
            res.end(
              JSON.stringify({
                success: false,
                error: err,
              }),
            );
          } else {
            // no error, send the data
            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(
              JSON.stringify({
                success: true,
                message: task,
              }),
            );
          }
        });
      }
    });
  });
};

const updateTask = (req: IncomingMessage, res: ServerResponse) => {
  // Read the data from the request
  let reqBody = "";
  req.on("data", (chunk) => {
    reqBody += chunk.toString();
  });
  // When the request is done
  req.on("end", () => {
    console.log("updateTask raw data: ", reqBody);
    // Parse the data
    let taskToBeUpdated: Task = JSON.parse(reqBody);
    if (!taskToBeUpdated.id) {
      res.writeHead(400, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({
          success: false,
          message: "Task id is required",
        }),
      );
      return;
    }
    // Read the fixtures/store.json file
    fs.readFile(jsonPath, "utf8", (err, data) => {
      // Check out any errors
      if (err) {
        // error, send an error message
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({
            success: false,
            error: err,
          }),
        );
      } else {
        // no error, get the current tasks
        let tasks: [Task] = JSON.parse(data);
        // find the task with the id
        let index = tasks.findIndex((t) => t.id == taskToBeUpdated.id);
        if (index === -1) {
          tasks.push(taskToBeUpdated);
        } else {
          // replace the task with the new one
          tasks[index] = taskToBeUpdated;
        }
        // write the new tasks array to the fixtures/store.json file
        fs.writeFile(jsonPath, JSON.stringify(tasks,null,2), (err) => {
          // Check out any errors
          if (err) {
            // error, send an error message
            res.writeHead(500, { "Content-Type": "application/json" });
            res.end(
              JSON.stringify({
                success: false,
                error: err,
              }),
            );
          } else {
            // no error, send the data
            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(
              JSON.stringify({
                success: true,
                message: taskToBeUpdated,
              }),
            );
          }
        });
      }
    });
  });
};

const deleteTask = (req: IncomingMessage, res: ServerResponse) => {
  // Read the data from the request
  let reqBody = "";
  req.on("data", (chunk) => {
    reqBody += chunk.toString();
  });
  // When the request is done
  req.on("end", () => {
    // Parse the data
    let taskToBeDeleted: Task = JSON.parse(reqBody);
    if (!taskToBeDeleted.id) {
      res.writeHead(400, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({
          success: false,
          message: "Task id is required",
        }),
      );
      return;
    }
    // Read the fixtures/store.json file
    fs.readFile(jsonPath, "utf8", (err, data) => {
      // Check out any errors
      if (err) {
        // error, send an error message
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({
            success: false,
            error: err,
          }),
        );
      } else {
        // no error, get the current tasks
        let tasks: [Task] = JSON.parse(data);
        // find the task with the id
        let index = tasks.findIndex((t) => t.id == taskToBeDeleted.id);
        if (index === -1) {
          res.writeHead(404, { "Content-Type": "application/json" });
          res.end(
            JSON.stringify({
              success: false,
              message: "Task not found",
            }),
          );
          return;
        }

        // remove the task
        tasks.splice(index, 1);
        // write the new tasks array to the fixtures/store.json file
        fs.writeFile(jsonPath, JSON.stringify(tasks,null,2), (err) => {
          // Check out any errors
          if (err) {
            // error, send an error message
            res.writeHead(500, { "Content-Type": "application/json" });
            res.end(
              JSON.stringify({
                success: false,
                error: err,
              }),
            );
          } else {
            // no error, send the data
            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(
              JSON.stringify({
                success: true,
                message: taskToBeDeleted,
              }),
            );
          }
        });
      }
    });
  });
};

export { getTasks, addTask, updateTask, deleteTask };
